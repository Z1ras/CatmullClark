...
#define new_struct(type, var) type var = (type)calloc(sizeof(type##_t), 1)
#define len(l) ((l)->n)
...
int list_push(list lst, void* e)
{
	...
	if (len(lst) >= lst->alloc) {
		...
		assert(p = (void**)realloc(lst->buf, sizeof(void*) * na));
...
void model_del(model m)
{
	...
	foreach(i, p, m->v) vertex_del((vertex)p);
	foreach(i, p, m->e) edge_del((edge)p);
	foreach(i, p, m->f) face_del((face)p);
...
int model_add_edge_i(model m, int i1, int i2)
{
	...
	return model_add_edge(m, (vertex)elem(m->v, i1), (vertex)elem(m->v, i2));
}
...
edge model_edge_by_v(model m, vertex v1, vertex v2)
{
	int i;
	edge e;
	for (i = 0, e = len(v1->e) ? (edge)elem(v1->e, i) : 0; i < len(v1->e); e = (edge)(++i == len(v1->e) ? 0 : elem(v1->e, i)))
	{
		if ((e->v[0] == v2 || e->v[1] == v2))
		return e;
	}
	i = model_add_edge(m, v1, v2);
	return (edge)elem(m->e, i);
}
...
void model_norms(model m)
{
	...
	coord_t d1, d2, norm;
	for (j = 0, f = len(m->f) ? (face)elem(m->f, j) : 0; j < len(m->f); f = (++j == len(m->f) ? 0 : (face)elem(m->f, j))) {
		n = len(f->v);
		for (i = 0, v = len(f->v) ? (vertex)elem(f->v, i) : 0; i < len(f->v); v = (++i == len(f->v) ? 0 : (vertex)elem(f->v, i))) {
			v0 = (vertex)elem(f->v, i ? i - 1 : n - 1);
			v1 = (vertex)elem(f->v, (i + 1) % n);
	...
	for (i = 0, v = len(m->v) ? (vertex)elem(m->v, i) : 0; i < len(m->v); v = (++i == len(m->v) ? 0 : (vertex)elem(m->v, i))) {
		for (j = 0, f = len(v->f) ? (face)elem(v->f, j) : 0; j < len(v->f); f = (++j == len(v->f) ? 0 : (face)elem(v->f, j)))
...
vertex edge_point(edge e)
{
	...
	if (!e->e_pt) {
		...
		if (!hole_edge(e)) {
			for (i = 0, f = len(e->f) ? (face)elem(e->f, i) : 0; i < len(e->f); f = (++i == len(e->f) ? 0 : (face)elem(e->f, i)))
...
vertex updated_point(vertex v)
{
	...
	if (hole_vertex(v)) {
		v->v_new->pos = v->pos;
		for (i = 0, e = len(v->e) ? (edge)elem(v->e, i) : 0; i < len(v->e); e = (++i == len(v->e) ? 0 : (edge)elem(v->e, i))) {
		...
		n = len(v->f);
		for (i = 0, f = len(v->f) ? (face)elem(v->f, i) : 0; i < len(v->f); f = (++i == len(v->f) ? 0 : (face)elem(v->f, i)))
...
model catmull(model m)
{
	...
	model nm = model_new();
	for (i = 0, f = len(m->f) ? (face)elem(m->f, i) : 0; i < len(m->f); f = (++i == len(m->f) ? 0 : (face)elem(m->f, i))) {
		for (j = 0, v = len(f->v) ? (vertex)elem(f->v, j) : 0; j < len(f->v); v = (++j == len(f->v) ? 0 : (vertex)elem(f->v, j))) {
			_get_idx(a, updated_point(v));
			_get_idx(b, edge_point((edge)elem(f->e, (j + 1) % len(f->e))));
			_get_idx(c, face_point(f));
			_get_idx(d, edge_point((edge)elem(f->e, j)));
...
void draw_wireframe(model m, GLfloat* color, GLfloat* hole_color)
{
	...
	for (i = 0, e = len(m->e) ? (edge)elem(m->e, i) : 0; i < len(m->e); e = (++i == len(m->e) ? 0 : (edge)elem(m->e, i))) {
...
void set_model()
{
	...
	foreach(i, p, models) model_del((model)p);
...
void render()
{
	...
	while (model_pos >= len(models))
		list_push(models, catmull((model)elem(models, len(models) - 1)));
	...
	if (model_pos && show_parent) {
		...
		draw_wireframe((model)elem(models, model_pos - 1), pcolor, hole2);
	}

	model m = (model)elem(models, model_pos);
